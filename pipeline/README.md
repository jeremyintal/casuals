...
- **Multi-team ledgers can over-connect explicitly partitioned sub-deals.** The parser correctly records what each franchise sent and received overall, but that does not prove every outgoing asset bought every incoming asset. Candidate #18 is the known example: Portland's official release says Keon Johnson/Nassir Little/Jusuf Nurkić returned Deandre Ayton/Toumani Camara, while Damian Lillard returned Jrue Holiday and picks. The generated Keon Johnson → Jrue Holiday edge is therefore invalid. Curators must inspect the official deal structure, not only the aggregate ledger.
